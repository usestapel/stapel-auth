...
class LoginNotificationServiceTests(TestCase):
    ...
    def test_is_new_device_true_when_unique(self):
        ...
        # login, which is "the only device", not a "new" one — see
        # test_login_alert_cold_start.
    ...
    def test_is_suspicious_ip_true_for_new_prefix(self):
        # Same fix: there's nothing to compare an "unfamiliar network"
        # against until the user has at least one prior login.

...
class MonitoringAuthCheckTests(APITestCase):
    def setUp(self):
        from stapel_auth.monitoring_proxy import MonitoringAuthCheckView
...
class GDPRProviderTests(TestCase):
    ...
    def test_serialize_dates_converts_datetimes(self):
        from stapel_auth.gdpr import _serialize_dates
...
class AuthSettingsTests(TestCase):
    def setUp(self):
        from stapel_auth.conf import AuthSettings
...
class AuditLogTests(APITestCase):
    ...
    def test_get_log_empty(self):
        resp = self.client.get(reverse('security_audit'))
        self.assertEqual(resp.status_code, 200)
    ...
    def test_get_log_pagination(self):
        ...
        resp = self.client.get(reverse('security_audit') + '?page=1')
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(len(resp.data['results']), 20)
        self.assertEqual(resp.data['next'], 2)

        resp2 = self.client.get(reverse('security_audit') + '?page=2')
...
class MagicLinkRequestTests(APITestCase):
    ...
    def test_request_nonexistent_email_returns_200(self):
        resp = self.client.post(reverse('magic_request'), {'email': 'nobody@example.com'}, format='json')
        self.assertEqual(resp.status_code, 200)
    ...
    def test_verify_no_token_redirects_to_error(self):
        resp = self.client.get(reverse('magic_verify'))
...
class RevokeSuspiciousTests(APITestCase):
    ...
    def test_no_token_redirects_to_error(self):
        resp = self.client.get(reverse('revoke_suspicious'))
        self.assertIn(resp.status_code, [302, 301])
        self.assertIn('invalid_link', resp['Location'])
...
class PasskeyListTests(APITestCase):
    ...
    def test_destroy_not_found_returns_404(self):
        resp = self.client.delete(reverse('passkey_destroy', kwargs={'pk': str(uuid.uuid4())}))
    ...
    def test_destroy_last_auth_method_returns_400(self):
        ...
        resp = self.client.delete(reverse('passkey_destroy', kwargs={'pk': str(pk.id)}))
        self.assertEqual(resp.status_code, 400)
...
class CaptchaSerializerTests(TestCase):
    """Integration tests for captcha enforcement in auth serializers."""

    def _phone_request_data(self, token=None):
        data = {'phone': '+12025550199'}
        if token is not None:
            data['captcha_token'] = token
        return data

    def _email_request_data(self, token=None):
        data = {'email': 'test@example.com'}
        if token is not None:
            data['captcha_token'] = token
        return data

    def test_captcha_disabled_no_secret_any_token_passes(self):
        """Without CAPTCHA_SECRET, captcha is disabled and any token is accepted."""
        from stapel_auth.otp.serializers import PhoneAuthRequestSerializer
        ser = PhoneAuthRequestSerializer(data=self._phone_request_data(token='garbage'))
        self.assertTrue(ser.is_valid(), ser.errors)

    def test_captcha_disabled_missing_token_passes(self):
        """Without CAPTCHA_SECRET, captcha_token field is not required."""
    ...
    def test_captcha_configured_invalid_token_raises_error(self):
        ...
        from stapel_auth.otp.serializers import PhoneAuthRequestSerializer
        from unittest.mock import patch
        with patch('stapel_core.captcha.backends.requests.post') as mock_post:
            mock_post.return_value.json.return_value = {'success': False}
            ser = PhoneAuthRequestSerializer(
                data=self._phone_request_data(token='bad-token'),
    ...
    def test_captcha_configured_valid_token_passes(self):
        ...
        with patch('stapel_core.captcha.backends.requests.post') as mock_post:
            ...
            ser = PhoneAuthRequestSerializer(
                data=self._phone_request_data(token='valid-token'),
                context={'request': None},
            )
            self.assertTrue(ser.is_valid(), ser.errors)
    ...
    def test_captcha_noop_backend_always_passes(self):
        ...
        from stapel_auth.otp.serializers import EmailAuthRequestSerializer
        ser = EmailAuthRequestSerializer(
            data=self._email_request_data(token='anything'),
            context={'request': None},
        )
        self.assertTrue(ser.is_valid(), ser.errors)

...
``AuthErrorKeysView`` existed since the port but no ``urls*.py`` mounted it —
...
def test_route_is_mounted_at_the_v1_canon_path():
    path = reverse("error-keys")
    assert path == "/auth/api/v1/error-keys/"
    assert resolve(path).func.view_class is AuthErrorKeysView
...
def test_staff_gets_the_registry_as_json():
    ...
    assert set(AUTH_ERRORS) <= set(payload)

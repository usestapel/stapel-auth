...
2. **POST `/auth/api/v1/password/login/`** — Verify the password; 423 when locked out; with TOTP enabled and PASSWORD_LOGIN_STEP_UP — a TOTP_REQUIRED response with a challenge_token
3. **POST `/auth/api/v1/totp/challenge/verify/`** — Optional step (only on TOTP_REQUIRED): exchange the challenge_token and the authenticator code for a JWT session

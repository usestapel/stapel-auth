...
The pytest urlconf mounts auth *bare* (``stapel_auth.urls`` → ``/password/login/``).
That is the repoint bug: the monolith aggregate — and therefore every frontend
projection and ``MANIFEST_TAGPREFIX="/auth/api/"`` — serves auth under its
canonical public API prefix, ``/auth/api/password/login/``.
...
``/auth/api/...`` paths (and the matching ``auth_api_*`` operationIds) and

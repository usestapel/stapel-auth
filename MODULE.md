...
- **Signup attribution** (`attribution.py`, `models.SignupAttribution`, axis `AUTH_SIGNUP_ATTRIBUTION`): every door that answers `REGISTERED` accepts an optional `attribution` object (`{click_id?, click_id_type?: gclid|gbraid|wbraid|yclid|fbclid|ttclid, captured_at, utm?}`) and stores it once against the new account — the OTP verifiers and `POST /oauth/login/` in the body, `GET /oauth/{provider}/authorize/` as flat query parameters parked in the flow state the callback already reads (the redirect flow has no body, and the identifier never travels through the provider). Stored **only when the call registers**; a login carries no new attribution, and an older `captured_at` never overwrites a newer one. The identifier is optional as long as `utm.source` names the channel — an email campaign or an aggregator puts no click id on the landing URL, and a click-id-only record would report every one of those accounts as direct traffic; what is refused is an identifier with no type (an offline upload names the field it posts to), a type naming no identifier, and a record carrying neither. Read back through `auth.signup_attribution`. Exists because browser-side conversion reporting depends on a session tie that a webmail tab, an OAuth round trip, thirty minutes or an unanswered consent banner each break silently — offline conversion import is the only channel that needs no cookie at conversion time, and the only one that can report a conversion the browser never witnessed. Storing it can never refuse an account: every write is wrapped and a failure logs instead of 500ing a sign-up.
...
| `auth.signup_attribution` | `{user_id}` → `{user_id, click_id, click_id_type, captured_at, utm{}, created_at, updated_at}` \| `null` | Whatever knows an account became a paying one (billing, a subscription job), reporting that conversion offline to the ad platform weeks after the browser that produced the click is gone. A Function and not an endpoint on purpose: the caller is a service, and a front end has no use for the click identifier and every reason not to hold it. `click_id`/`click_id_type` are both blank on a tags-only record (channel known, nothing to upload offline — a different fact from no row at all). `null` is the **ordinary** answer — most accounts have no attribution — and must be read as "nothing to upload", never as "look it up somewhere else"; an unparseable `user_id` answers `null` too, so an unknown id cannot look like an outage |
| `auth.user_projection` | `{user_id}` → `{found, user?}` | The pull half of the user projection (0.34.2), for a consumer that names a second user it has never met and cannot wait for `user.created` to have already arrived — the account may predate the consumer, or the handler may have missed the fact while it aged out of the stream. `user`, when present, is exactly `projection_payload(user)` (`serialize_user_to_jwt_data` verbatim) — the same payload the event carries and `get_or_create_user_from_jwt` materialises a shadow row from, so mirroring on first contact and mirroring from the event never drift. `found: false` for an id nobody ever issued, one an erasure has since deleted (the row is gone either way, so the two cases are rightly indistinguishable), and any malformed id — never raised. A **deactivated** account still has a row: it answers `found: true` with `is_active: false` in the payload, and this Function does not act on that flag itself — the caller's own gate does, the same division `get_or_create_user_from_jwt` already holds for a token's own claim |
